...
from backend.services import synthesize_all_languages, synthesize_speech
...
def run_async(coro):
    ...
    If there's a running event loop (e.g., in some interactive environments),
    run the coroutine on a background thread to avoid RuntimeError.
    """
    try:
        return asyncio.run(coro)
    except RuntimeError:
        # Event loop is running in this thread — run in a separate thread.
...
def split_audio(file_path):
    ...
    try:
        while start_second < duration:
            ...
            chunk_audio = chunk_audio.set_channels(TARGET_CHANNELS).set_frame_rate(
                TARGET_SAMPLE_RATE
            )
            chunk_path = chunk_dir / f"chunk-{index:04d}.wav"
            chunk_audio.export(
                str(chunk_path), format="wav", parameters=["-acodec", "pcm_s16le"]
            )
...
def create_demo():
    # Optionally import gradio at runtime to avoid static analysis errors when gradio
    # is not installed in all environments. Use importlib to dynamically load the
    # module only when this demo is invoked.
    import importlib
    ...
    if gr is None:
        raise RuntimeError("Gradio is not installed; create_demo() requires gradio.")
    ...
    with gr.Blocks(title="Voice2PDF") as demo:
        ...
        with gr.Tab("Audio to PDF"):
            ...
            progress_bar = gr.Slider(
                minimum=0, maximum=100, value=0, label="Progress", interactive=False
            )
            ...
            start_btn.click(
                ...
                lambda job_id: (
                    gr.update(visible=True) if job_id else gr.update(visible=False)
                ),
                job_id_output,
                progress_bar,
            )
            ...
            job_id_output.change(
                poll_status, job_id_output, [status_output, progress_bar, pdf], every=2
            )
        with gr.Tab("PDF to Audio"):
            pdf_input = gr.File(label="PDF", file_types=[".pdf"])
            pdf_language = gr.Dropdown(
                list(LANGUAGES), value="English", label="Language"
            )
            audio_output = gr.Audio(label="Audio")
            audio_file = gr.File(label="Download")
            gr.Button("Generate Audio").click(
                pdf_to_audio, [pdf_input, pdf_language], [audio_output, audio_file]
            )
